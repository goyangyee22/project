...
async function deleteDocument(collectionName, docId) {
  try {
    // 해당 문서를 가져옵니다.
    const boardDoc = await getDocs(firestoredoc(dbService, collectionName, docId));
    ...
    // 게시글의 작성자 정보 가져오기
    const writerDocId = boardDoc.docs[0].data().docId;
    ...
    if (writerDocId === currentDocId) {
      await deleteDoc(firestoreDoc(dbService, collectionName, docId));
      return true;
    } else {
